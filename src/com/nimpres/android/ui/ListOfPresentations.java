		...
		searchButton.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View view) {
				...
				Log.d("ListOfPresentations", "peer list size: "
						+ NimpresObjects.peerPresentations.size());

				for (int i = 0; i < NimpresObjects.peerPresentations.size(); i++) {
					Log.d("ListOfPresentations", "checking: "
							+ NimpresObjects.peerPresentations.get(i)
									.getPresenterName());
					if (presenterIDString
							.equals(NimpresObjects.peerPresentations.get(i)
									.getPresenterName())) {
						item.add(NimpresObjects.peerPresentations.get(i));
						Log.d("ListOfPresentations", "added item to list"
								+ NimpresObjects.peerPresentations.get(i)
										.getPresentationName());
					}
				}
				NimpresObjects.internetPresentations = PeerStatus
						.getInternetPresentations(NimpresObjects.presenterName,
								NimpresObjects.presenterPassword,
								presenterIDString);
				for (int i = 0; i < NimpresObjects.internetPresentations.size(); i++) {
					Log.d("ListOfPresentations-Internet", "checking: "
							+ NimpresObjects.internetPresentations.get(i)
									.getPresenterName());
					if (presenterIDString
							.equals(NimpresObjects.internetPresentations.get(i)
									.getPresenterName())) {
						item.add(NimpresObjects.internetPresentations.get(i));
						Log.d("ListOfPresentations-Internet",
								"added item to list"
										+ NimpresObjects.internetPresentations
												.get(i).getPresentationName());
					}
				}
				ArrayAdapter<PeerStatus> presentationList = new ArrayAdapter<PeerStatus>(
						view.getContext(), R.layout.row, item);
	...
	protected void onListItemClick(ListView l, View v, int position, long id) {
		...
		selected.setPositiveButton("OK", new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int which) {
				...
				NimpresObjects.updateSource = ListOfPresentations.peerStatus
						.getSource();
				NimpresObjects.presentationID = ListOfPresentations.peerStatus
						.getPresentationID();
				JoinPresentation.updateID();
				Log.d("ListOfPresentations-return",
						"presentation id:"
